...
test_that("util_nps_tbbase validates sf object inputs", {
  ...
  tbconserv <- create_mock_sf(crs = 6443)

  expect_error(
    util_nps_tbbase(tblu, tbsoil, tbconserv),
...
test_that("util_nps_tbbase processes successfully with valid inputs", {
  ...
  tbconserv <- poly6443(category = "Conservation")
  ...
  tbbase2_conserv_mock <- poly6443(bay_seg = "TS1", basin = "Basin1", drnfeat = "Feature1",
                                   entity = "City1", conservation = FALSE)

  tbbase3_mock <- poly6443(bay_seg = "TS1", basin = "Basin1", drnfeat = "Feature1",
                           entity = "City1", conservation = FALSE, FLUCCSCODE = 1100)
  ...
  stub(util_nps_tbbase, "util_nps_union", function(...) {
    ...
    if (union_call_count == 3) return(tbbase2_conserv_mock)
    if (union_call_count == 4) return(tbbase3_mock)
    if (union_call_count == 5) return(tbbase4_mock)
  })

  result <- util_nps_tbbase(tblu, tbsoil, tbconserv, verbose = FALSE)

  expect_s3_class(result, "data.frame")
  expect_false("geometry" %in% names(result))
  expect_true("area_ha" %in% names(result))
  expect_true("conservation" %in% names(result))
  expect_equal(union_call_count, 5)

  union_call_count <- 0
  expect_output(
    util_nps_tbbase(tblu, tbsoil, tbconserv, verbose = TRUE),
    paste(
      "Combining drainage basins with sub-watersheds...",
      "Combining results with TBNMC jurisdictions...",
      "Combining results with conservation lands...",
