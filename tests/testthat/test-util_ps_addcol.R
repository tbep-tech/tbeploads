dat <- read.table(psdompth, skip = 0, sep = '\t', header = TRUE)

test_that("util_ps_addcol function add TN", {

  datchk <- subset(dat, select = -c(Total.N, Total.N.Unit))
  result <- util_ps_addcol(datchk)
  expect_true("Total.N" %in% names(result))
  expect_true("Total.N.Unit" %in% names(result))

})

test_that("util_ps_addcol function add TP", {
...
test_that("util_ps_addcol function adds TSS", {
...
test_that("util_ps_addcol function add BOD", {
...
test_that("util_ps_addcol function uses CBOD for BOD if present", {
