test_that("anlz_ad returns correct columns for segment and month summary", {
  ...
  expect_true(all(c("Year", "Month", "source", "bayseg", "tn_load", "tp_load", "hy_load") %in% colnames(result)))
})
...
test_that("anlz_ad returns correct columns for segment and year summary", {
  ...
  expect_true(all(c("Year", "source", "bayseg", "tn_load", "tp_load", "hy_load") %in% colnames(result)))
})
