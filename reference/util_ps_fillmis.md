...
Input data frame as is if no missing values, otherwise missing data
filled as described above.

## Details

Missing concentration data are replaced with the average for the outfall
in a given year. All flow data are also floored at zero. Rows with
missing flow data are assigned 0 for all data. Rows with zero flow are
assigned concentration of zero.
