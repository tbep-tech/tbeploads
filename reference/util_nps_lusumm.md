...
Data frame with summarized loading data based on user-supplied
arguments, loading data in tons per month or year depending on the
`summtime` argument, and hydrologic load in million cubic meters per
month or year depending on the `summtime` argument.
