...
Do **not** use literal non-ASCII characters (e.g. `→`, `–`, `×`,
typographic quotes) in:
...
The Rd unicode escape syntax is `\u{XXXX}` (4-digit hex) inside a
roxygen comment; it renders correctly in both HTML and PDF help. Do
**not** use the R-string escape `\uXXXX` (without braces) — that is for
character literals inside R code, not Rd markup.
