...
The raw Verna download also carries NADP completeness flags, `Criteria1` (percent of days with a valid sample) and `Criteria3` (percent of precipitation captured by valid samples). `util_prepverna()` treats a month as missing (same as the raw `-9` value) if either flag falls below `mincrit` (default 75), before any gap-filling occurs, so that low-quality months are filled using recent same-month means rather than used as-is. This screen is skipped if `Criteria1`/`Criteria3` are absent from the input file.
