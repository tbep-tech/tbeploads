...
* `util_nps_getusgsflow()`: Gets USGS flow data for NPS calculations, used in `anlz_nps_getflow()`.
* `util_nps_getextflow()`: Gets external flow data and contacts (Lake Manatee, Tampa Bypass, and Bell Shoals), used in `anlz_nps_getflow()`.
* `util_nps_getwq()`: Gets water quality data for NPS gaged calculations (for `mancopth`, `pincopth`, and Hillsborough County data).
* `util_nps_getrain()`: Gets rainfall data for NPS ungaged calculations, as provided in `rain`.
