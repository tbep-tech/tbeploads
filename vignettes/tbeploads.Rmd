...
Using `anlz_ad()` to estimate AD load is done as follows, where `[rain](../reference/rain.html)` is the rain data and `vernafl` is the path to the Verna Wellfield data. 
...
Ungaged (unmonitored basins) estimates are obtained using `anlz_nps_ungaged()`. The approach combines spatial land use data, rainfall patterns, hydrologic modeling, and empirical relationships to estimate monthly nutrient and sediment loads. The function requires combined spatial data for bay segment, basin, entity jurisdiction, land use data, and soils (using `[tbbase](../reference/tbbase.html)`), rainfall data (using `[rain](../reference/rain.html)`), and flow data (obtained with `util_nps_getflow()`). A preprocessed USGS flow data frame can be provided using the `[usgsflow](../reference/usgsflow.html)` argument to avoid re-downloading the data. 

The first step is updating the combined spatial data if any of the input datasets have changed.  The required inputs are land use (`[tblu2020](../reference/tblu2020.html)` or `[tblu2023](../reference/tblu2023.html)`), soil data (`[tbsoil](../reference/tbsoil.html)`), jurisdiction (`[tbjuris](../reference/tbjuris.html)`), and sub-basin data (`[tbsubshed](../reference/tbsubshed.html)`).  The function `util_nps_tbbase()` combines these datasets into a single spatial object that is used for ungaged load estimation using `util_nps_union()`.  The function requires GDAL to be installed and accessible in the system PATH, or the path to GDAL binaries can be provided using the `gdal_path` argument.
...
Then, the combined layer, `[tbbase](../reference/tbbase.html)`, can be created (takes an hour or two).
...
The `[tbbase](../reference/tbbase.html)` data object is also included with the package for convenience.
...
* `util_nps_getusgsflow()`: Gets USGS flow data for NPS calculations, used in `util_nps_getflow()`. A preprocessed USGS flow data frame can be provided using the `[usgsflow](../reference/usgsflow.html)` argument to avoid re-downloading the data.
...
* `util_getrain()`: Gets rainfall data for NPS ungaged calculations, as provided in `[rain](../reference/rain.html)`.
* `util_nps_tbbase()`: Gets the combined spatial data layer for NPS ungaged calculations, as provided in `[tbbase](../reference/tbbase.html)`.
