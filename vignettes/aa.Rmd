...
`total_h2o` is the annual NPS + IPS + DPS water load for that basin (matching the SAS `ratio1_2224` denominator). `mean_h2o_9294` is the 1992-1994 baseline total basin water volume from `hydro_baseline`. Effective loads are averaged over `yrrng` as `sum / length(yrrng)`, so missing years contribute zero rather than being excluded.
...
where `basin_total_h2o` is the same total basin water load (NPS + IPS + DPS) used in the NPS path, matching the SAS `ratio1_2224` construction.
