...
#' @param tbconserv Optional sf object of conservation lands polygons (e.g., FNAI Conservation Lands).
#'   When provided, each row in the output gains a logical \code{conservation} column indicating whether
#'   that land area falls within a conservation boundary. If \code{NULL} (default), the output omits the
#'   \code{conservation} column and all areas are treated as non-conservation.
...
#'
#' @returns A summarized data frame containing the union of all inputs showing major bay segment, sub-basin (basin), drainage feature (drnfeat), jurisdiction (entity), land use/land cover (FLUCCSCODE), CLUCSID, IMPROVED, hydrologic group (hydgrp), area in hectares, and optionally \code{conservation} (logical). These represent all relevant spatial combinations in the Tampa Bay watershed.
#'
#' @details
...
#' }
util_nps_tbbase <- function(tblu, tbsoil, tbconserv = NULL, gdal_path = NULL,
